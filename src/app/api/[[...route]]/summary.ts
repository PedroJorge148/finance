  ...
  async (c) => {
    ...
    async function fetchFinancialData(
      ...
      return await db
        .select({
          ...
          remaining: sum(transactions.amountInCents).mapWith(Number),
        })
        .from(transactions)
    ...
    const category = await db
      .select({
        ...
        value: sql`SUM(ABS(${transactions.amountInCents}))`.mapWith(Number),
      })
      .from(transactions)
      .innerJoin(accounts, eq(transactions.accountId, accounts.id))
      ...
      .where(
        and(
          ...
          lt(transactions.amountInCents, 0),
          gte(transactions.date, startDate),
          lte(transactions.date, endDate),
        ),
      )
      .groupBy(categories.name)
      .orderBy(desc(sql`SUM(ABS(${transactions.amountInCents}))`))
